...
static int pmw3610_set_performance(const struct device *dev, bool enabled) {
    ...
    // On wake (re-enable after idle/sleep) the PMW3610 emits bogus motion
    // bursts. Drop the first few bursts so they never move the cursor.
    if (enabled && !data->performance_enabled) {
        data->drop_motion_bursts = PMW3610_DROP_BURSTS_AFTER_WAKE;
        data->dx = 0;
        data->dy = 0;
    }
    data->performance_enabled = enabled;
    ...
    if (config->force_awake) {
        ...
        // Keep force-awake sticky: releasing it on idle lets the sensor fall
        // into rest mode, whose wake bursts are bogus and move the cursor.
...
#if CONFIG_PMW3610_ALT_MOTION_DELTA_MAX > 0
    if (x > CONFIG_PMW3610_ALT_MOTION_DELTA_MAX ||
